...
# [개발 요구사항 & 흐름도]  
## 1. 요구사항  
-시작화면,12개의 스테이지,엔딩화면 총 14개의 화면이 있다.  
-시작화면에는 게임 시작과 게임 종료 버튼이 있다.  
-게임 시작 버튼을 누르면 첫 스테이지부터 게임이 시작되며 게임 종료 버튼을 누르면 게임이 종료된다  
-게임이 시작되면 첫 스테이지부터 마지막 스테이지까지 순서대로 진행이 되며 마지막 스테이지를 클리어 할 시 게임을 완료하게 된다.  
-중간에 게임오버를 당하게 되면 시작화면으로 돌아갈지 아니면 해당 스테이지부터 재도전 할 지 선택할 수 있다.  
-조작은 키보드 WASD키로 이동을 하며 스페이스바 키로 구르기를 할 수 있으며 마우스로 적을 조준하고 마우스 좌클릭으로 총알을 발사한다.  
-총알은 엄폐물에 막힐 수 있다.  
-스킬은 쉬프트키로 사용할 수 있다.  
-화면 우측 위에는 미니맵이 있으며 좌측 위에는 플레이어의 상태가 있다. 화면 오른쪽 하단에는 현재 무기의 상태를 보여주며 화면 왼쪽 하단은 스킬의 쿨타임을 보여준다.  
## 2. 시간별 흐름도 flowchart  
